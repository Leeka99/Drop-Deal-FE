...
권한: 해당 주문의 `BUYER` 또는 인증된 비회원 주문자
...
회원과 비회원 모두 현재 진행 중인 주문을 취소할 수 있다. 비회원은 `Guest-Order-Token` 헤더가 필요하며 토큰에 연결된 본인 주문만 취소할 수 있다. 취소가 접수되면 주문 상태를 `CANCELLED`로 변경하고 결제 금액 전액 환불을 요청한다. 배송 시작, 수령 완료, 이미 취소된 주문 등 진행 중 상태가 아닌 주문의 취소 가능 여부는 운영 정책에 따라 백엔드가 판단한다.
...
`비회원 주문 조회` 진입 링크는 로그인하지 않은 사용자의 첫 화면에서만 노출하며 로그인 페이지와 로그인 상태 화면에는 노출하지 않는다.
...
비회원 결제 배송지 입력 화면에는 `로그인 후 배송지 가져오기` 기능을 제공한다. 로그인 성공 후 사용자가 결제 중이던 상품의 checkout 화면으로 복귀시키며, 로그인만으로 배송지를 자동 적용하지 않고 구매자가 `기본 배송지 사용`을 선택해야 불러온다. 로그인 이동 전 비회원 입력값은 유지되지 않을 수 있음을 안내한다.
