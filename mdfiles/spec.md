...
재고떨이 상품은 일반 상품보다 더 큰 할인율이 적용될 수 있고, 특정 조건을 만족하면 참여자 중 랜덤 5%에게 다음 구매에 사용할 수 있는 30% 할인 쿠폰을 지급한다.
...
이 절은 현재 프론트 구현 기준으로 추가된 최신 기능을 정리한다.

### 25.1 판매자 계정 구매 제한

판매자 계정은 서비스 화면을 둘러볼 수 있지만 실제 구매는 할 수 없다.

적용 규칙:

* 판매자 계정은 상품 상세를 열람할 수 있다.
* 판매자 계정은 `/products/[id]/checkout` 에서 결제할 수 없다.
* 판매자 계정이 `/mypage/orders` 로 접근하면 `/seller/sales` 로 이동한다.
* 헤더의 `내 참여` 위치는 판매자 계정일 때 `판매 내역` 으로 바뀐다.
* 승인된 판매자 계정만 판매자 센터에 진입할 수 있다.

판매자 구매 제한 화면 문구:

```txt
판매자 계정은 상품을 구경할 수 있지만 직접 구매할 수 없습니다.
```

### 25.2 판매자 전용 판매 내역

판매자 전용 메뉴로 `판매 내역` 페이지를 제공한다.
...
구매자 계정은 기존처럼 `내 공동구매 참여 내역` 을 본다.
...
* 구매자 계정만 내 참여 목록을 본다.
* 판매자 계정은 내 참여 대신 판매 내역으로 이동한다.

### 25.4 구매 차단 UX

판매자 계정이 상품 상세 또는 결제 페이지에 들어왔을 때의 동작:

* 상품 상세는 정보 열람만 허용한다.
* 공동구매 참여 버튼 대신 판매자 안내 영역을 보여준다.
* 결제 페이지에서는 참여 제한 안내를 보여준다.
* 안내 영역에는 판매 내역 보기, 판매자 센터 이동 버튼을 제공한다.
