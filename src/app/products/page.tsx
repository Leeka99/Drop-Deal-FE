...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ type?: string }> }) {
  ...
  const products = query.type === "clearance"
    ? allProducts.filter((p) => p.type === "CLEARANCE")
    : query.type === "free"
  ...
  return (
    <div className="shell">
      ...
      <div className="toolbar">
        ...
        {["전체","일반 공동구매","재고떨이","완전무료!","쿠폰 이벤트","마감 임박","할인율 높은순"].map((label,index)=><button className={`filter ${index === (query.type === "clearance" ? 2 : query.type === "free" ? 3 : 0) ? "active":""}`} key={label}>{label}</button>)}
      ...
      {products.length > 0
        ...
        : <div style={{ paddingBottom:70 }}><EmptyProducts /></div>}
    </div>
  );
}
